...
import { _cityDeleteAction, _cityListAction, _citySearchAction } from '@/actions/CityActions';
...
export default function CityList({ dbData, provincesData }) {
    const {cityState, cityDispatch } = AdminContextState()
    ...
    useEffect(() => {
        cityDispatch({type: 'ADD_DATA', payload: {
            items: dbData?.data,
    ...
    async function paginateHandler(url) {
        try{
            const res = await _cityPaginateAction(url)
            cityDispatch({type: 'ADD_DATA', payload: {
                items: res?.data,
    ...
    async function getSearchData() {
        if(!search) {
            ...
            try{
            const res = await _citySearchAction(search);
            cityDispatch({type: 'ADD_DATA', payload: {
                items: res?.data,
                prevURL: res?.links.prev,
                nextURL: res?.links.next,
    ...
    async function getData() {
        try{
            const res = await _cityListAction();
            cityDispatch({type: 'ADD_DATA', payload: {
                items: res?.data,
                prevURL: res?.links.prev,
                nextURL: res?.links.next,
    ...
    async function deleteData(id) {
        try{
            const res = await _cityDeleteAction(id);
            if(res.status == 1) {
                await getData();
  ...
  return (
    ...
    <section className='w-[100%] pt-4'>
        ...
        <section className="mx-auto w-[90%] lg:overflow-hidden overflow-scroll pb-[2rem]">
            <section className='lg:w-[100%] w-[70rem]'>
                ...
                <div className='mx-auto w-[100%] text-lg py-2 flex items-center justify-start font-bold font-white bg-gray-200 '>
                    ...
                    <div className='w-[30%] border-r border-white px-3 py-2'>PROVINCE</div>
                ...
                { cityState?.items?.length > 0 ?
                    ...
                    <div key={key} className='mx-auto w-[100%] py-2 flex items-center justify-start border-b border-x border-gray-300'>
                        ...
                        <div className='w-[15%] px-3 py-2 text-end flex items-center justify-end gap-3 text-xl'>
                            <Link title='View' href={`/admin/city/${i?.id}`}> 
        ...
        <section className='w-[90%] mx-auto pb-[4rem] flex items-center justify-end gap-3'>
            ...
            {cityState?.prevURL &&
            <button
                onClick={() => paginateHandler(cityState.prevURL)}
            ...
            {cityState?.nextURL &&
            <button 
                onClick={() => paginateHandler(cityState.nextURL)}
                className='group px-4 py-2 flex items-center justify-center gap-2 border border-gray-700 text-gray-700'>
                <span>Next</span>
