...
import { _faqDeleteAction, _faqListAction, _faqSearchAction } from '@/actions/FaqActions';
...
export default function FaqList({ dbData }) {
    const {faqState, faqDispatch } = AdminContextState()
    const [isModal, setIsModal] = useState(false)
    const [search, setSearch] = useState('');
    const [isSearch, setIsSearch] = useState(false);

    useEffect(() => {
        faqDispatch({type: 'ADD_DATA', payload: {
            items: dbData?.data,
    ...
    async function deleteData(id) {
        try{
            const res = await _faqDeleteAction(id);
            if(res.status == 1) {
                await getData();
  ...
  return (
    ...
    <section className='w-[100%] pt-4'>
        ...
        <section className="mx-auto w-[90%] lg:overflow-hidden overflow-scroll pb-[2rem]">
            <section className='lg:w-[100%] w-[70rem]'>
                ...
                <div className='mx-auto w-[100%] text-lg py-2 flex items-center justify-start font-bold font-white bg-gray-200 '>
                    <div className='w-[45%] border-r border-white px-3 py-2'>TITLE</div>
                ...
                { faqState?.items?.length > 0 ?
                    ...
                    <div key={key} className='mx-auto w-[100%] py-2 flex items-center justify-start border-b border-x border-gray-300'>
                        ...
                        <div className='w-[15%] px-3 py-2 text-end flex items-center justify-end gap-3 text-xl'>
                            <Link title='View' href={`/admin/faq/${i?.id}`}> 
        ...
        <section className='w-[90%] mx-auto pb-[4rem] flex items-center justify-end gap-3'>
            ...
            {faqState?.prevURL &&
            <button
                onClick={() => paginateHandler(faqState.prevURL)}
            ...
            {faqState?.nextURL &&
            <button 
                onClick={() => paginateHandler(faqState.nextURL)}
                className='group px-4 py-2 flex items-center justify-center gap-2 border border-gray-700 text-gray-700'>
                <span>Next</span>
    ...
    <FaqAddModal getData={getData} isModal={isModal} setIsModal={setIsModal} />
    </>
  )
}
