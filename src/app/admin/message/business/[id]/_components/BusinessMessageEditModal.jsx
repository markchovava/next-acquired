"use client";
import { _businessMessageStatusAction } from '@/actions/BusinessMessageActions';
...
export default function BusinessMessageEditModal({id, getData, domData, isModal, setIsModal}) {
    ...
    async function postData() {
        ...
        try{
            const res = await _businessMessageStatusAction(formData);
            if(res.status == 1) {
                await getData();
                   ...
                   <h2 className='font-serif text-[2.6rem] mb-6 text-center border-b border-gray-300'>
                    ...
                    <div className='w-[100%] mb-6'>
                        ...
                        <select
                            ...
                            <option 
                            value='Unread' 
                            /* selected={data?.status == 'Unread' && 'selected'} */
                            >Unread</option>
                            <option 
                            value='Read' 
                            /* selected={data?.status == 'Read' && 'selected'} */
                            >Read</option>
                        </select>
                        {errMsg?.status &&
                          <p className='text-red-600'>{errMsg?.status}</p>}
                    </div>
