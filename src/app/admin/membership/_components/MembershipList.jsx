...
import { _membershipDeleteAction, _membershipListAction, _membershipSearchAction } from '@/actions/MembershipActions';
...
export default function MembershipList({ dbData }) {
    const {membershipState, membershipDispatch } = AdminContextState()
    const [isModal, setIsModal] = useState(false)
    const [search, setSearch] = useState('');
    const [isSearch, setIsSearch] = useState(false);

    useEffect(() => {
        membershipDispatch({type: 'ADD_DATA', payload: {
            items: dbData?.data,
    ...
    async function paginateHandler(url) {
        try{
            const res = await _membershipPaginateAction(url)
            membershipDispatch({type: 'ADD_DATA', payload: {
                items: res?.data,
    ...
    async function getSearchData() {
        ...
        try{
            const res = await _membershipSearchAction(search);
            membershipDispatch({type: 'ADD_DATA', payload: {
                items: res?.data,
                prevURL: res?.links?.prev,
                nextURL: res?.links?.next,
    ...
    async function getData() {
        try{
            const res = await _membershipListAction();
            membershipDispatch({type: 'ADD_DATA', payload: {
                items: res?.data,
                prevURL: res?.links.prev,
                nextURL: res?.links.next,
    ...
    async function deleteData(id) {
        try{
            const res = await _membershipDeleteAction(id);
            if(res.status == 1) {
                await getData();
  ...
  return (
    ...
    <section className='w-[100%] pt-4'>
        ...
        <section className="mx-auto w-[90%] lg:overflow-hidden overflow-scroll pb-[2rem]">
            <section className='lg:w-[100%] w-[70rem]'>
                ...
                { membershipState?.items?.length > 0 ?
                    membershipState?.items?.map((i, key) => (
                    <div key={key} className='mx-auto w-[100%] py-2 flex items-center justify-start border-b border-x border-gray-300'>
                        ...
                        <div className='w-[15%] px-3 py-2 text-end flex items-center justify-end gap-3 text-xl'>
                            <Link title='View' href={`/admin/membership/${i?.id}`}> 
        ...
        <section className='w-[90%] mx-auto pb-[4rem] flex items-center justify-end gap-3'>
            {/* PREVIOUS */}
            {membershipState?.prevURL &&
            <button
                onClick={() => paginateHandler(membershipState.prevURL)}
            ...
            {/* NEXT */}
            {membershipState?.nextURL &&
            <button 
                onClick={() => paginateHandler(membershipState.nextURL)}
                className='group px-4 py-2 flex items-center justify-center gap-2 border border-gray-700 text-gray-700'>
                <span>Next</span>
